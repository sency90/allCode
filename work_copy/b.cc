...
//Dependencies: ./string.txt, ./dictionary.txt
...
void hashing(string &line, int idx, ll x) {
    ...
    if(h.count(init)) {
        if(str[h[init]]!=line) {
            ...
            puts("Caution!! Dictionary is not consisted of distinct strings!!");
            printf("The numbers of line: %d, %d were conflicted.\n", h[init], idx);
        }
    } else h[init]=idx;
}
...
int main() {
    ...
    for(int i=0; getline(infd, line); i++) {
        if(!i) {
            ...
            return 0&puts("Dictionary words must be equal to each other.\n");
        }
        str.push_back(line);
        printf("%s\n", line.c_str());
        hashing(line, i, x);
