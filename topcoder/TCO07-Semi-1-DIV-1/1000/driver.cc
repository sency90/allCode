...
vector <int>	positions;
		...
		__in >> __expected >> n >> positions;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << n << ", " << positions << std::endl;
		std::cout.flush();

		__timer_start();

		WorstCaseQuicksort __object;
		vector <int> __actual = __object.worstInput(n, positions);
