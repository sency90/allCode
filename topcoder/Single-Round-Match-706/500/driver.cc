...
int	p;
		...
		__in >> __expected >> x >> f >> d >> p;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << x << ", " << f << ", " << d << ", " << p << std::endl;
		std::cout.flush();

		__timer_start();

		SellingFruits __object;
		int __actual = __object.maxDays(x, f, d, p);
