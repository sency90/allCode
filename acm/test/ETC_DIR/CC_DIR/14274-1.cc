...
int main() {
    ...
    int cnt=1;
    for(auto &i: coy) yy[i.second]=++cnt;
    sort(co.begin(), co.end());
    for(int i=0; i<n; i++) d[i]=Integer(1);
    for(int len=2; len<=3; len++) {
        for(int i=0; i<=cnt; i++) tree[i]=Integer(0);
        //memset(tree,0,sizeof(tree));
        for(int i=0; i<n; i++) {
            for(int j=yy[co[i].second]; j<=cnt; j+=j&-j) {
                tree[j]+=d[i];
            }
            d[i]=Integer(0);
            for(int j=yy[co[i].second]-1; j>=1; j-=j&-j) {
                d[i]+=tree[j];
            }
        }
    }
